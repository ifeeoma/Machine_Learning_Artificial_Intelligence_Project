...
sublist=MS.subset_list(cut_ends,29846,26)
...
estimator.fit(training_data, training_target)
...
predictions_probs = estimator.predict_proba(test_data)
predictions_list = []
...
lines = []
lines.append('activation = '+act+ ', epochs = '+str(epochs)+', batch = '+str(batch_size)+', neurons = '+str(neurons)+'gives an accuracy of ' +str(acc))
print("activation function =",act, ', epochs =', epochs, ', batch size =', batch_size, ', neurons = ', neurons, ', gives an accuracy of', acc)
